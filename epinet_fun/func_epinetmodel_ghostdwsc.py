...
def layer1_multistream_ghostdwsc(input_dim1, input_dim2, input_dim3, filt_num):
    seq = Sequential()
    ...
    for i in range(3):
        seq.add(ghostmodule.GhostModule((seq.output_shape[1], seq.output_shape[2], seq.output_shape[3]), filt_num, ratio,
                                    convKernel, dwKernel))
        seq.add(Activation('relu', name='S1_relu1%d' % (i)))
        seq.add(ghostmodule.GhostModuleDWSC((seq.output_shape[1], seq.output_shape[2], seq.output_shape[3]), filt_num, ratio,
                                    convKernel, dwKernel))
...
def layer2_merged_ghostdwsc(input_dim1, input_dim2, input_dim3, filt_num, conv_depth):
    ...
    for i in range(conv_depth):
        seq.add(
            ghostmodule.GhostModule((seq.output_shape[1], seq.output_shape[2], seq.output_shape[3]), filt_num, ratio,
                                    convKernel, dwKernel))
        seq.add(Activation('relu', name='S2_relu1%d' % (i)))
        seq.add(
            ghostmodule.GhostModuleDWSC((seq.output_shape[1], seq.output_shape[2], seq.output_shape[3]), filt_num, ratio,
                                    convKernel, dwKernel))
...
def layer3_last_ghostdwsc(input_dim1, input_dim2, input_dim3, filt_num):
...
def define_epinet_ghostdwsc(sz_input, sz_input2, view_n, conv_depth, filt_num, learning_rate):
    tFlops = 0
    tMem = 0
    tParam = 0
    tMem_req = 0
    ...
    mid_90d = layer1_multistream_ghostdwsc(sz_input, sz_input2, len(view_n), int(filt_num))(input_stack_90d)
    mid_0d = layer1_multistream_ghostdwsc(sz_input, sz_input2, len(view_n), int(filt_num))(input_stack_0d)
    mid_45d = layer1_multistream_ghostdwsc(sz_input, sz_input2, len(view_n), int(filt_num))(input_stack_45d)
    mid_M45d = layer1_multistream_ghostdwsc(sz_input, sz_input2, len(view_n), int(filt_num))(input_stack_M45d)

    ''' Merge layers '''
    mid_merged = concatenate([mid_90d, mid_0d, mid_45d, mid_M45d])

    ''' Merged layer : Conv - Relu - Conv - BN - Relu '''
    mid_merged_ = layer2_merged_ghostdwsc(sz_input - 6, sz_input2 - 6, int(4 * filt_num), int(4 * filt_num), conv_depth)(
        mid_merged)

    ''' Last Conv layer : Conv - Relu - Conv '''
    output = layer3_last_ghostdwsc(sz_input - 20, sz_input2 - 20, int(4 * filt_num), int(4 * filt_num))(mid_merged_)
...
def evaluateEPINet_ghostdwsc():
    ...
    model = define_epinet_ghostdwsc(image_w, image_h,
