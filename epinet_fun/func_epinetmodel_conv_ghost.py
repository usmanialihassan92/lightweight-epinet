...
def layer1_multistream_cg(input_dim1, input_dim2, input_dim3, filt_num):
    seq = Sequential()
...
def layer2_merged_cg(input_dim1, input_dim2, input_dim3, filt_num, conv_depth):
...
def layer3_last_cg(input_dim1, input_dim2, input_dim3, filt_num):
...
def define_epinet_cg(sz_input, sz_input2, view_n, conv_depth, filt_num, learning_rate):
    tFlops = 0
    tMem = 0
    tParam = 0
    tMem_req = 0
    ...
    mid_90d = layer1_multistream_cg(sz_input, sz_input2, len(view_n), int(filt_num))(input_stack_90d)
    mid_0d = layer1_multistream_cg(sz_input, sz_input2, len(view_n), int(filt_num))(input_stack_0d)
    mid_45d = layer1_multistream_cg(sz_input, sz_input2, len(view_n), int(filt_num))(input_stack_45d)
    mid_M45d = layer1_multistream_cg(sz_input, sz_input2, len(view_n), int(filt_num))(input_stack_M45d)

    ''' Merge layers '''
    mid_merged = concatenate([mid_90d, mid_0d, mid_45d, mid_M45d])

    ''' Merged layer : Conv - Relu - Conv - BN - Relu '''
    mid_merged_ = layer2_merged_cg(sz_input - 6, sz_input2 - 6, int(4 * filt_num), int(4 * filt_num), conv_depth)(
        mid_merged)

    ''' Last Conv layer : Conv - Relu - Conv '''
    output = layer3_last_cg(sz_input - 20, sz_input2 - 20, int(4 * filt_num), int(4 * filt_num))(mid_merged_)
...
def evaluateEPINet_cg():
    ...
    model = define_epinet_cg(image_w, image_h,
