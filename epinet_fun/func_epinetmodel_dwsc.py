...
def layer1_multistream_dwsc(input_dim1,input_dim2,input_dim3,filt_num):
    seq = Sequential()
...
#    seq.add(Reshape((input_dim1,input_dim12,input_dim3),input_shape=(input_dim1, input_dim2, input_dim3,1)))
    for i in range(3):
        seq.add(SeparableConv2D(int(filt_num),(2, 2),input_shape=(input_dim1, input_dim2, input_dim3),padding=paddingType,name='conv_dw_%d' % (i)))
...
def layer2_merged_dwsc(input_dim1,input_dim2,input_dim3,filt_num,conv_depth):
    ''' Merged layer : Conv - Relu - Conv - BN - Relu '''
    
    seq = Sequential()
    
    for i in range(conv_depth):
        seq.add(SeparableConv2D(filt_num,(2,2), padding=paddingType,input_shape=(input_dim1, input_dim2, input_dim3), name='S2_c1%d' % (i) ))
...
def layer3_last_dwsc(input_dim1,input_dim2,input_dim3,filt_num):
    ''' last layer : Conv - Relu - Conv ''' 
    
    seq = Sequential()
    
    for i in range(1):
        seq.add(SeparableConv2D(filt_num,(2,2), padding=paddingType,input_shape=(input_dim1, input_dim2, input_dim3), name='S3_c1%d' %(i) )) # pow(25/23,2)*12*(maybe7?) 43 3
...
def define_epinet_dwsc(sz_input,sz_input2,view_n,conv_depth,filt_num,learning_rate):
    ...
    mid_90d=layer1_multistream_dwsc(sz_input,sz_input2,len(view_n),int(filt_num))(input_stack_90d)
    mid_0d=layer1_multistream_dwsc(sz_input,sz_input2,len(view_n),int(filt_num))(input_stack_0d)
    mid_45d=layer1_multistream_dwsc(sz_input,sz_input2,len(view_n),int(filt_num))(input_stack_45d)
    mid_M45d=layer1_multistream_dwsc(sz_input,sz_input2,len(view_n),int(filt_num))(input_stack_M45d)

    ''' Merge layers ''' 
    mid_merged = concatenate([mid_90d,mid_0d,mid_45d,mid_M45d])

    ''' Merged layer : Conv - Relu - Conv - BN - Relu '''
    mid_merged_=layer2_merged_dwsc(sz_input-6,sz_input2-6,int(4*filt_num),int(4*filt_num),conv_depth)(mid_merged)

    ''' Last Conv layer : Conv - Relu - Conv '''
    output=layer3_last_dwsc(sz_input-20,sz_input2-20,int(4*filt_num),int(4*filt_num))(mid_merged_)
...
def evaluateEPINet_dwsc():
    ...
    model = define_epinet_dwsc(image_w, image_h,
